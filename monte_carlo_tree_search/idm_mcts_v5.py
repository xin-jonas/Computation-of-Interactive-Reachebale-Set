...
class MonteCarloTreeSearchV:
    ...
    def eval_action_reward(self, path):
        ...
        J_p = self.Base.cal_distance_to_obstacle(ego_state, time_step, sim_scenario=self.sim_scenario)
    ...
    def eval_terminal_reward(self, node: MCTSNode = None, path=None):
        ...
        # collision penalty
        if not self.Base.is_collision_free(path, sim_scenario=self.sim_scenario):
