...
class MonteCarloTreeSearchV:
    ...
    def expansion(self, node):
        ...
        if len(node.unexpanded_actions) == 0:
            ...
            if node.expanded_children:
                ...
                return node
            return sub_node
        # expanded one action, which is not attend before
        while True:
            # all actions have been chosen
            ...
            # check collision in the given scenario
            if self.Base.is_collision_free(path_translated):
                # if we cant find a collision free action after 9 times random choice
                # we break the choice
                # print('------new child generate ------')
                # print('expanded-state: ' + repr(path_translated[-1]))
                break
        ...
        # print('len path translated:' + repr(len(path_translated)))
    ...
    def eval_terminal_reward(self, node: MCTSNode = None, path=None):
        ...
        # collision penalty
        if not self.Base.is_collision_free(path):
    ...
    def is_terminal(self, node=None, path=None, collision_check=True):
        ...
        return time or not collision_free
